...
def brighten(pixel):
    return(b_intensity(pixel[0]), b_intensity(pixel[1]), b_intensity(pixel[2]))
...
def b(image_surf):
    ...
    for x in range(rows):
        for y in range(cols):
            pixels3d[x,y] = brighten(pixels3d[x,y])

def d_intensity(intensity):
    if (intensity <= 10):
        return 0
    else:
        return intensity-10
...
def darken(pixel):
    return(d_intensity(pixel[0]), d_intensity(pixel[1]), d_intensity(pixel[2]))
...
# g: modifies image pixel array of image_surf in place
